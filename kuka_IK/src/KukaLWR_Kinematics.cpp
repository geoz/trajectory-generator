...
#define DEBUG_IK 1
#define USE_LWR 1 // 1 for LWR,  0 for paper's robot
...
#else
// Robot from paper:
const double KukaLWR_Kinematics::JNT_LIMITS [] = {90.0*PI/180.0,
		45.0*PI/180.0,
		120.0*PI/180.0,
		180.0*PI/180.0,
		90.0*PI/180.0,
...
// WARNING: Avoid Singularities. This IK solver doesn't work (yet) for singularities like:
// 			Wrist singularities: Wrist above (x,y)=(0.0), paper proposes solution
// Tested and it works:  poseCircle  (as long as circle is outside of singularities
// 					     poseBackAndForth
...
bool KukaLWR_Kinematics::ikSolverAnalytical7DOF( geometry_msgs::Pose & poseDsr, std::vector<double> & jntPosDsr)
{
	...
	if ( (poseDsr.position.z >= (D3+D5+D7+0.31)) || ( poseDsr.position.x == 0.0 && poseDsr.position.y ==0.0 && poseDsr.position.z >= (D3+D5+D7+0.31) )  )
	{
		#if DEBUG_IK
		cout << "Zero configuration" << endl;
		#endif
	...
	// KUKA_LWR constants	// I am not sure about the values of these constants. Be careful!!!
	double dbs = 0.31;
	double dse = D3;
	double dew = D5;
	double dwt = D7;
	...
	KDL::Rotation Aw = R43.Inverse()*AsT*R70des;  //Not sure if I can use inverse instead of transpose
	...
	// Printing sets:
#if DEBUG_IK
	std::cout << endl <<"array3D i size = " << array3D.size() << endl;
	// print psi_i sets
	for ( i=0; i<array3D.size(); i++)
	{
		std::cout<<endl<< endl<< "set:"<<i <<endl ;
		for ( j=0; j<set_number[i]; j++)
		{
			std::cout << endl;
			for ( k=0; k<2; k++)
			{
				std::cout  <<" "<< array3D[i][j][k] * 180/PI << " " ;
			}
		}
	}
	std::cout<< endl << endl;
